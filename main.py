...
# Глобальные переменные для TkinterDnD
TkinterDnD = None
DND_FILES = None
...
class LowLineRemoverApp(ctk.CTk):
    def __init__(self):
        ...
        # Настройка окна
        self.title("Программа для удаления символов подчеркивания из имен файлов")
    ...
    def setup_ui(self):
        ...
        # Область перетаскивания (drag and drop)
        self.drop_frame = ctk.CTkFrame(self.file_selection_frame, fg_color="#e0e0e0", corner_radius=10)
        self.drop_frame.pack(fill=tk.BOTH, expand=True, padx=20, pady=20)
        
        dnd_text = "Перетащите файлы сюда или нажмите для выбора папки"
        if not self.dnd_enabled:
            dnd_text = "Нажмите для выбора папки с файлами"
        ...
        # Привязка событий к области перетаскивания
        self.drop_frame.bind("<Button-1>", self.select_folder)
        self.drop_label.bind("<Button-1>", self.select_folder)
        
        # Настройка DnD, если доступно
        if self.dnd_enabled:
            try:
                # Регистрируем drop_frame, а не self
                self.drop_frame.drop_target_register(DND_FILES)
                self.drop_frame.dnd_bind('<<Drop>>', self.drop)
                print("Функция перетаскивания файлов включена")
            except Exception as e:
                print(f"Ошибка при настройке функции перетаскивания: {e}")
                self.dnd_enabled = False
                self.drop_label.configure(text="Нажмите для выбора папки с файлами")
    ...
    def drop(self, event):
        try:
            ...
            if not filtered_files:
                messagebox.showinfo("Информация", "Среди перетащенных файлов нет файлов с символом подчеркивания.")
                return
                
            self.selected_files = filtered_files
            self.update_file_list()
        except Exception as e:
            messagebox.showerror("Ошибка", f"Не удалось обработать перетащенные файлы: {str(e)}")

    def parse_drop_data(self, data):
        file_paths = []
        
        # Обработка строки с путями файлов
        # Данные могут приходить в разных форматах в зависимости от ОС
        try:
            if data:
                # Для Windows - убираем фигурные скобки и приводим слеши к единому формату
                data = data.replace('{', '').replace('}', '')
                
                # Разделяем пути (они могут быть разделены пробелами или переносами строк)
                for item in data.split():
                    # Очистка и декодирование пути
                    item = item.strip()
                    if item:
                        # Для Windows проверяем наличие символа ":" и исправляем слеши
                        if os.name == 'nt':
                            item = item.replace('\\', '/')
                        ...
                        if os.path.isfile(item):
                            ...
                            # Если перетаскивается каталог, добавляем все файлы из него
                            for root, _, filenames in os.walk(item):
                                for filename in filenames:
                                    file_path = os.path.join(root, filename)
                                    file_paths.append(file_path)
        except Exception as e:
            print(f"Ошибка при обработке перетащенных файлов: {e}")
...
if __name__ == "__main__":
    try:
        if TkinterDnD:
            # Если tkinterdnd2 доступен, используем его
            root = TkinterDnD.Tk()
            ...
            # Если tkinterdnd2 не установлен, запускаем обычное приложение
            print("Предупреждение: библиотека tkinterdnd2 не установлена. Функция перетаскивания файлов будет недоступна.")
